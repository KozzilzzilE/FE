package com.example.fe.ui.theme

import androidx.compose.ui.graphics.Color

// ── 피그마 디자인 시스템 ──────────────────────────────────

// Background
val BgPrimary    = Color(0xFF1C1917)   // 메인 배경
val BgSurface    = Color(0xFF292524)   // 카드/서피스
val BgElevated   = Color(0xFF44403C)   // 높은 서피스 (구분선, 탭)
val BgDivider    = Color(0xFF57534E)   // 구분선

// Text
val TextPrimary   = Color(0xFFFAFAF9)  // 기본 텍스트
val TextSecondary = Color(0xFFA8A29E)  // 보조 텍스트
val TextMuted     = Color(0xFF78716C)  // 흐린 텍스트

// Primary (Amber)
val Primary       = Color(0xFFF59E0B)  // 메인 강조색
val PrimaryDim15  = Color(0x26F59E0B)  // 15% opacity
val PrimaryDim40  = Color(0x66F59E0B)  // 40% opacity
val PrimaryDim80  = Color(0xCCF59E0B)  // 80% opacity

// Semantic
val Success  = Color(0xFF22C55E)
val Error    = Color(0xFFFB2C36)
val ErrorAlt = Color(0xFFEF4444)
val Blue     = Color(0xFF3B82F6)
val Cyan     = Color(0xFF06B6D4)

// Code editor background
val CodeBgDark = Color(0xFF0D1117)

// Topic icon backgrounds (semi-transparent)
val IconBgAmber   = Color(0x26F59E0B)  // 해시 등
val IconBgSalmon  = Color(0x26FFBFB2)  // 스택/큐 등
val IconBgYellow  = Color(0x26F2F3C7)  // 기타

// Legacy (기존 코드 호환)
val Purple80      = Color(0xFFD0BCFF)
val PurpleGrey80  = Color(0xFFCCC2DC)
val Pink80        = Color(0xFFEFB8C8)
val Purple40      = Color(0xFF6650a4)
val PurpleGrey40  = Color(0xFF625b71)
val Pink40        = Color(0xFF7D5260)

// Legacy color aliases (old tokens → new design tokens)
val CardBg             = BgSurface
val CodeBg             = CodeBgDark
val CodeText           = Cyan
val DividerColor       = BgDivider
val GrayText           = TextSecondary
val TitleText          = TextPrimary
val BodyText           = TextPrimary
val TopBarTitle        = TextPrimary
val TopBarSub          = TextSecondary
val Mint               = Success
val ProgressBlue       = Blue
val ProgressTrack      = BgElevated
val BlankBorder        = BgDivider
val BlankBorderSelected = Primary
val CorrectText        = Success
val WrongText          = Error
val PageBg             = BgPrimary
val BlankBox           = BgElevated
val BlankBoxSelected   = PrimaryDim40
val BlankTextFilled    = TextPrimary
val CheckBtnBg         = Primary
val CheckBtnText       = BgPrimary
val ChoiceBorder       = BgDivider
val EmptyBorder        = BgDivider
val PrimaryButtonBlue  = Blue
val SelectedAnswerBg   = PrimaryDim15
